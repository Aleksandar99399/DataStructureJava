...
public class TheMatrix {
    ...
    public void solve() {
        ...
        while (!coordinates.isEmpty()){
            ...
            if (isInBounds(row+1,col)&&this.matrix[row+1][col]==startChar){
                coordinates.offer(new int[]{row+1,col});
            }
            if (isInBounds(row-1,col)&&this.matrix[row-1][col]==startChar){
                coordinates.offer(new int[]{row-1,col});
            }
            if (isInBounds(row,col+1)&&this.matrix[row][col+1]==startChar){
    ...
    private boolean isInBounds(int i, int col) {
        return !isOutOfBounds(i,col);
    }
